...
module.exports.init = app => {
  ...
  app.get('/growingStock/:countryIso', (req, res) => {
    ...
    const areaValues = getFraValues('forestCharacteristics', req.params.countryIso)
    const growingStock = repository.readGrowingStock(req.params.countryIso)

    Promise.all([areaValues, growingStock])
      .then(result => res.json({areaValues: result[0].fra, values: result[1]}))
