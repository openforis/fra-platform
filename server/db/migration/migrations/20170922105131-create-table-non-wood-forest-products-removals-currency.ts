...
exports.up = function (db: any) {
  const filePath = path.join(
    __dirname,
    'sqls',
    '20170922105131-create-table-non-wood-forest-products-removals-currency-up.sql'
  )
...
exports.down = function (db: any) {
  const filePath = path.join(
    __dirname,
    'sqls',
    '20170922105131-create-table-non-wood-forest-products-removals-currency-down.sql'
  )
  return new Promise(function (resolve, reject) {
    fs.readFile(filePath, { encoding: 'utf-8' }, function (err: any, data: any) {
      if (err) return reject(err)
