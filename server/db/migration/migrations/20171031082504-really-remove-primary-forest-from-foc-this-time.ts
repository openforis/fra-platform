...
exports.up = function (db: any) {
  const filePath = path.join(__dirname, 'sqls', '20171031082504-really-remove-primary-forest-from-foc-this-time-up.sql')
...
exports.down = function (db: any) {
  const filePath = path.join(
    __dirname,
    'sqls',
    '20171031082504-really-remove-primary-forest-from-foc-this-time-down.sql'
  )
  return new Promise(function (resolve, reject) {
    fs.readFile(filePath, { encoding: 'utf-8' }, function (err: any, data: any) {
      if (err) return reject(err)
