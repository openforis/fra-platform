...
exports.up = function (db: any) {
  const filePath = path.join(__dirname, 'sqls', '20170928081213-create-disturbances-up.sql')
...
exports.down = function (db: any) {
  const filePath = path.join(__dirname, 'sqls', '20170928081213-create-disturbances-down.sql')
  return new Promise(function (resolve, reject) {
    fs.readFile(filePath, { encoding: 'utf-8' }, function (err: any, data: any) {
      if (err) return reject(err)
      console.log(`received data: ${data}`)
