...
CREATE VIEW ndp_view AS
  (SELECT o.country_iso,
          v.year,
          v.data_source_methods,
          CASE
              WHEN o.draft_id IS NULL THEN FALSE
              ELSE TRUE
          END AS draft,
   ...
   FROM public.odp o
   LEFT OUTER JOIN public.odp_version v ON v.id = CASE
                                                      WHEN o.draft_id IS NULL THEN o.actual_id
                                                      ELSE o.draft_id
                                                  END
   LEFT OUTER JOIN odp_class c ON c.odp_version_id = v.id
