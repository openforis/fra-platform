...
const getCountryRoles = (countryIso, userInfo) =>
  R.filter(R.propEq("countryIso", countryIso))(userInfo.roles);
const getCountryRole = (countryIso, userInfo) =>
  getCountryRoles(countryIso, userInfo)[0];
...
const roleForCountry = (countryIso, userInfo) => {
  ...
  assert(
    rolesForCountry.length < 2,
    `Ambiguous roles found for user ${userInfo} and country ${countryIso}`
  );
...
const hasUserRole = (countryIso, userInfo, roleObj) =>
  roleForCountry(countryIso, userInfo).role === roleObj.role;

const isReviewer = (countryIso, userInfo) =>
  hasUserRole(countryIso, userInfo, reviewer) ||
  hasUserRole(countryIso, userInfo, administrator);
const isNationalCorrespondent = (countryIso, userInfo) =>
  hasUserRole(countryIso, userInfo, nationalCorrespondent);
const isAlternateNationalCorrespondent = (countryIso, userInfo) =>
  hasUserRole(countryIso, userInfo, alternateNationalCorrespondent);
const isCollaborator = (countryIso, userInfo) =>
  hasUserRole(countryIso, userInfo, collaborator);
const hasNoRole = (countryIso, userInfo) =>
  hasUserRole(countryIso, userInfo, noRole);
