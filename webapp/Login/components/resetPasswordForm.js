...
const ResetPasswordNotFound = () => (
  <div className="alert-error">
    ...
    <div>
      Ooops. It looks like the link you clicked is expired or not valid.
      <br />
      To reset your password go to the{' '}
      <a href="/login" style={{ fontWeight: 'bold', color: 'white' }}>
        login page
      </a>
    </div>
  </div>
...
const ResetPasswordForm = (props) => {
  const { status = '', resetPassword = {}, changePasswordResponse = {}, changePassword, findResetPassword } = props
    ...
    <div className="login__form">
      ...
      {changePasswordResponse.error ? (
        <div className="alert-error">
          ...
          <div>{changePasswordResponse.error}</div>
        </div>
      ) : null}
      <div className="login__buttons">
        <button
          ...
          type="button"
          onClick={() => {
            changePassword(resetPassword.uuid, resetPassword.user.id, password, password2)
          }}
        >
          Change password
        </button>
