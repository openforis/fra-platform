...
const NewVersionForm = (props) => {
  ...
  const minDate = format(
    new Date(), 'yyyy-MM-ddThh:mm', { awareOfUnicodeTokens: true }
  )
  const maxDate = format(
    new Date(new Date().getUTCFullYear() + 2 + ''), 'yyyy-MM-ddThh:mm', { awareOfUnicodeTokens: true }
  )
  ...
  return <form onSubmit={onSubmit}>
    ...
    <input
      min={minDate}
      max={maxDate}
      onChange={onChange} type="datetime-local" name="timestamp" /> <br />
