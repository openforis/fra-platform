...
export const isARegionalFocalPoint = (user: User, cycle: Cycle): boolean => {
  return user?.roles?.some(
    (userRole: UserRole<never>) =>
      userRole?.role === RoleName.REGIONAL_FOCAL_POINT && userRole?.cycleUuid === cycle.uuid
  )
}
