...
const tableName = 'table_1_1b'
const colName = 'forest_area_1990'
...
export const buildCases = (props: Props): Array<TableValidationTestCase> => {
  ...
  return [
    ...
    // A forest area with no forest type reported is invalid
    {
      cell,
      data: [forestArea('1000')],
      expected: differentFromForestArea('1000.00'),
      name: `${equalToSum}: forest area without forest types is invalid`,
    },
    // Empty forest types don't count as 0, so not even a forest area of 0 matches them
    {
      cell,
      data: [forestArea('0')],
      expected: differentFromForestArea('0.00'),
      name: `${equalToSum}: forest area of 0 without forest types is invalid`,
    },
    ...
    // A sum over or below the forest area is invalid
    {
      cell,
    ...
    // Reported zeroes count as values, so zero forest types match a forest area of 0
    {
      cell,
