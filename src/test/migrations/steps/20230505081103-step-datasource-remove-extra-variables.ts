...
const _removeVariable = async (
  { sectionName, variableName, assessment, cycle }: { assessment: Assessment; cycle: Cycle; sectionName: string; variableName: string },
  client: BaseProtocol
) => {
...
export default async (client: BaseProtocol) => {
  ...
  // 2a
  await _removeVariable({ sectionName: 'growingStockComposition', variableName: 'mostRecentYear', cycle, assessment }, client)
  // 4a
  ...
  // 7a
  await _removeVariable({ sectionName: 'nonWoodForestProductsRemovals', variableName: 'totalValue', cycle, assessment }, client)
}
