...
const Description: React.FC<Props> = (props) => {
  ...
  return (
    <div className="fra-description__header-row">
      ...
      {open && !Objects.isEmpty(text) && dataSourceTextReadOnly && <p>{t('nationalDataPoint.dataSource2025ExplanatoryText')}</p>}
      {showMarkdownEditor && (
        <div className="fra-description__preview">
          <EditorWYSIWYG value={text} onChange={(content) => onChange({ ...commentableDescriptionValue, text: content })} />
        </div>
      )}
      {showPreview && (
        <div className="fra-description__preview">
