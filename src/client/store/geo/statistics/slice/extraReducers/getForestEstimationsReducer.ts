...
export const getForestEstimationsReducer = (builder: ActionReducerMapBuilder<GeoStatisticsState>): void => {
  builder.addCase(getForestEstimations.fulfilled, (state, action) => {
    ...
    Objects.setInPath({ obj: state, path: ['geoStatistics', 'forestEstimations'], value: forestEstimations })
    Objects.setInPath({ obj: state, path: ['geoStatistics', 'loading'], value: false })
    Objects.setInPath({ obj: state, path: ['geoStatistics', 'errorKey'], value: null })
  })
  builder.addCase(getForestEstimations.pending, (state) => {
    Objects.setInPath({ obj: state, path: ['geoStatistics', 'loading'], value: true })
    Objects.setInPath({ obj: state, path: ['geoStatistics', 'errorKey'], value: null })
  })
  builder.addCase(getForestEstimations.rejected, (state, action) => {
    ...
    Objects.setInPath({ obj: state, path: ['geoStatistics', 'loading'], value: false })
    Objects.setInPath({ obj: state, path: ['geoStatistics', 'errorKey'], value: errorKey })
  })
}
