...
type Returned = { assessment: AssessmentBase; cycle: Cycle }

export const createCycle = async (props: Props, client: BaseProtocol = DB): Promise<Returned> => {
  const { assessment: assessmentBase, cycleSource, name, user, withCountries } = props

  return client.tx(async (t) => {
    const assessment = await AssessmentRepository.getOne({ id: assessmentBase.id }, t)
