...
export const getCreateOrReplaceViewCountryUserSummary = (props: { assessment: Assessment; cycle: Cycle }) => {
  ...
  return `
    ...
    select distinct on (u.uuid, coalesce(ur.country_iso, ui.country_iso))
      ...
      u.status,
      to_jsonb(ur.*) as role,
      case when to_jsonb(ur.*) is null then to_jsonb(ui.*) else null end as invitation
    from users u
    left join users_role ur on 
      u.uuid = ur.user_uuid 
      and ((ur.cycle_uuid = '${cycle.uuid}' and ur.assessment_uuid = '${assessment.uuid}') 
        or ur.role = 'ADMINISTRATOR')
    left join users_invitation ui on 
      u.uuid = ui.user_uuid 
      and ((ui.cycle_uuid = '${cycle.uuid}' and ui.assessment_uuid = '${assessment.uuid}') 
        or ur.role = 'ADMINISTRATOR')
