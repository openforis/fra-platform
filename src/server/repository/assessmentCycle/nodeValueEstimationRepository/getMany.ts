  ...
  return client.map<NodeValuesEstimation>(
    `
      with estimations_ranked as (
        select e.uuid,
          row_number() over (partition by jsonb_object_keys(e.variables) order by a.time desc) as row_number
        from ${schemaCycle}.node_values_estimation e
          left join public.activity_log a on a.target ->> 'uuid' = e.uuid::text
