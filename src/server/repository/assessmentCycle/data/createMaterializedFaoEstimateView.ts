...
export const createMaterializedFaoEstimateView = async (props: Props): Promise<void> => {
...
with country_nodes as (select n.country_iso,
     ...
     country_node_exts as (select n.country_iso,
                           ...
                           from ${schemaCycle}.node_ext n
                                    ...
                                    left join ${schemaAssessment}."table" t
                                              on t.props ->> 'name' = n.props ->> 'tableName'
                           where t.props ->> 'name' = $1
     ...
     region_values as (select cr.region_code as country_iso,
                              cv.variable_name,
                              cv.col_name,
                              jsonb_build_object('raw', text(coalesce(sum((cv.value ->> 'raw')::numeric), 0)),
                                                 'faoEstimate', true)
                                             as value
