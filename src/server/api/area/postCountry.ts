...
export const postCountry = async (
  req: CycleRequest<{ notifyUsers: string; direction?: 'next' | 'previous' }, { country: Country; message: string }>,
  res: Response
) => {
  try {
    const { countryIso, assessmentName, cycleName, notifyUsers, direction } = req.query
